...
$oVIP->selfurl = 'qhr_usr_pwd_chg.php';
...
if ( isset($_POST['ok']) )
{
  ...
  if ( empty($qti_error) )
  {
    ...
    $row = $oDB->Getrow();
    if ($row['countid']==0) $qti_error='Old password'.S.' invalid';
  }
  ...
  if ( empty($qti_error) )
  {
    ...
    $oVIP->EndMessage(NULL,'Security profile saved',$_SESSION[QT]['skin_dir'],2);
  }
}
...
<p style="text-align:right">Old_password&nbsp;<input type="password" id="title" name="title" size="20" maxlength="24"/></p>
<p style="text-align:right">New_password&nbsp;<input type="password" id="newpwd" name="newpwd" size="20" maxlength="24"/></p>
<p style="text-align:right">Confirm_password&nbsp;<input type="password" id="conpwd" name="conpwd" size="20" maxlength="24" onKeyUp="handle_keypress(event,\'ok\')"/></p>
